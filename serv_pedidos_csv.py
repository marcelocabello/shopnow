...
def crear_pedido(p: PedidoRegistro):
    # 1. Validar Cliente (Interoperabilidad)
    try:
    ...
    try:
        ...
        if not any(int(prod['id_producto']) == p.id_producto for prod in res_p.json()):
            raise HTTPException(status_code=400, detail="El producto no existe en el catálogo")
    except requests.exceptions.ConnectionError:
        raise HTTPException(status_code=503, detail="Departamento de Productos no disponible")

    # 3. Generar ID autoincremental
    def leer_pedidos():
    ...
    return {"mensaje": "Pedido registrado y validado en archivo CSV", "id_pedido": siguiente_id}